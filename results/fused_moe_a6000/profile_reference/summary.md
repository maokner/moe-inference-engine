...
MoE mode: reference.
Clean median decode latency: 55.147 ms (18.12 tok/s).
Instrumented total: 58.001 ms per step using CPU-scope wall time under CPU-activity torch.profiler.
...
|---|---:|---:|---:|
| MoE routing/top-2 selection | 39.969 | 68.9% | 54.0 |
| QKV and attention work | 4.784 | 8.2% | 6.0 |
| Expert MLP execution | 4.079 | 7.0% | 12.0 |
| LayerNorm/residual/output | 3.420 | 5.9% | 20.0 |
| Route-weight combination | 1.821 | 3.1% | 12.0 |
| Fused MoE decode | 0.000 | 0.0% | 0.0 |

Combined MoE work: 45.869 ms per step (79.1% of instrumented decode time).
The combined MoE path dominates instrumented decode time, so it is the next kernel target.
Largest component scope: MoE routing/top-2 selection at 39.969 ms per step (68.9% of instrumented decode time).
The existing per-expert loop itself has 1.879 ms per step of profiler-unattributed Python CPU time (3.4% of clean median latency); its nested tensor operations and kernel launches are reported in the component rows.
Pure Python loop overhead is not the dominant bottleneck.
The largest individual CPU operator is aten::nonzero at 27.295 ms per step across 48.0 calls per step.
CPU operator self time: 45.164 ms per step across 1659.0 calls per step.
